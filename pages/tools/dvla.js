  ...
  return (
    <>
      <Head>
        ...
        <meta name="keywords" content="DVLA, vehicle enquiry, reg lookup, check number plate, MOT status, Tax status, number plate search, vehicle registration" />
              ...
              {vehicleData && (
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-4 md:my-6">
                    <div className="bg-white rounded-xl shadow-md text-left p-6 md:p-8 flex items-center justify-between dark:bg-gray-800">
                      <h3 className="text-xl font-bold">Tax Status</h3>
                      <p className={`rounded-md text-white py-2 px-3 ${vehicleData.taxStatus === 'Taxed' ? 'bg-green-500' : 'bg-red-500'}`}>
                        {vehicleData.taxStatus === 'Taxed' ? 'Taxed' : 'Not Taxed'}
                      </p>
                    </div>
                    <div className="bg-white rounded-xl shadow-md text-left p-6 md:p-8 flex items-center justify-between dark:bg-gray-800">
                      <h3 className="text-xl font-bold">MOT Status</h3>
                      {vehicleData.motStatus === 'No details held by DVLA' ? (
                        <p className="rounded-md bg-gray-500 text-white py-2 px-3">N/A</p>
                      ) : (
                        <p className={`rounded-md text-white py-2 px-3 ${vehicleData.motStatus === 'Valid' ? 'bg-green-500' : 'bg-red-500'}`}>
                          {vehicleData.motStatus === 'Valid' ? 'Valid' : 'Invalid'}
                        </p>
                      )}
                    </div>
                  </div>
