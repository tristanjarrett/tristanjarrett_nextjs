        ...
        <div className="container px-4 mx-auto flex-grow">
          <div className="py-16 lg:py-24 flex flex-col lg:flex-row justify-center items-center lg:space-x-20 text-center">
            <div className="lg:w-3/5">
              ...
              {vehicleData && (
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-6 text-white">
                    <div className={`rounded-md p-4 mb-2 ${vehicleData.taxStatus === 'Taxed' ? 'bg-green-500' : 'bg-red-500'}`}>
                      <h3 className="text-xl font-bold mb-2">Tax Status</h3>
                      <p className="font-bold">
                        {vehicleData.taxStatus === 'Taxed' ? 'Taxed' : 'Not Taxed'}
                      </p>
                    </div>
                    <div className={`rounded-md p-4 mb-2 ${vehicleData.motStatus === 'Valid' ? 'bg-green-500' : 'bg-red-500'}`}>
                      <h3 className="text-xl font-bold mb-2">MOT Status</h3>
                      <p className="font-bold">
                        {vehicleData.motStatus === 'Valid' ? 'Valid' : 'Invalid'}
                      </p>
                    </div>
                  </div>
                  <div className="bg-white rounded-xl shadow-lg text-left p-6 dark:bg-gray-800">
